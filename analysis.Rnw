...
qplot(updrs, pv, data=cs, geom="point") + geom_smooth(method="lm")
...
GetTestByGroup <- function(d, var1, var2) {
  ret.0 <- GetCorTestValues(filter(d, Group==0) , var1, var2)
  ret.1 <- GetCorTestValues(filter(d, Group==1) , var1, var2)
...
qplot(updrs, value, data=cs.melt.list$ptp1, geom="point", color=Group) + 
      facet_wrap(~variable, scales="free_y") + geom_smooth(method="lm")
GetTestResultsbyResponseVariable(cs, "updrs", variables.list$ptp[1:5])
...
\noindent
There doesn't look like there is any separation between the groups that is
significant. All the p-values above are bigger than 0.05.
